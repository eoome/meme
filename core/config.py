            ...
            with open(_CONFIG_FILE, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if indent == 0 and ':' in stripped:
                        ...
                        if not val:
                            # 纯 section header，等待子 key
                            current_section = key
                            raw.setdefault(key, {})
                        else:
                            # 一级 key-value（如 "data_dir: /path"）
                            raw[key] = _parse_yaml_value(val)
                            current_section = None
                    elif indent > 0 and current_section and ':' in stripped:
                        # 二级 key-value（如 "  timeout: 8"）
                        key = stripped.partition(':')[0].strip()
                        val = stripped.partition(':')[2].strip()
                        if key:
                            raw[current_section][key] = _parse_yaml_value(val)
            return raw
...
def _parse_yaml_value(val: str):
    """将 YAML 值字符串转为 Python 类型（int/float/bool/str）"""
    if not val:
        return val
    # 布尔值
