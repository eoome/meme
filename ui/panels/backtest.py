...
class StockDetailPanel(QFrame):
    ...
    def update_result(self, result_dict):
        ...
        extra_parts = []
        # 回测区间
        bt_start = r.get('backtest_start_date', '')
        bt_end = r.get('backtest_end_date', '')
        if bt_start and bt_end:
            extra_parts.append(f"📅 {bt_start} ~ {bt_end}")
        extra_parts += [
        ...
        # 信号来源统计
        src_stats = r.get('signal_source_stats', {})
        ml_ratio = r.get('ml_signal_ratio', 0)
        if src_stats:
            ml_count = src_stats.get('ml', 0)
            rule_count = src_stats.get('rule', 0)
            total_preds = sum(src_stats.values())
            if total_preds > 0:
                extra_parts.append(
                    f"信号来源: 🧠ML {ml_count}次({ml_ratio:.0%})  📏规则 {rule_count}次"
                )
...
class TradesTable(QFrame):
    ...
    def _init_ui(self):
        ...
        self._table.setHorizontalHeaderLabels(["时间", "方向", "价格", "数量", "金额", "盈亏", "信号来源"])
...
class BacktestPanel(QFrame):
    ...
    def _build_backtest_params(self):
        ...
        """)
        grid_outer = QVBoxLayout(grid)
        grid_outer.setSpacing(10)
        ...
        row_bottom.setSpacing(24)

        grid_outer.addLayout(row_top)
        grid_outer.addLayout(row_bottom)
        ...
        row_bottom.addLayout(col_mc)

        # 列6: 回测区间
        from PyQt5.QtWidgets import QDateEdit
        col_date = QVBoxLayout()
        col_date.setSpacing(8)
        t_date = QLabel("📅 回测区间")
        t_date.setFont(QFont("Microsoft YaHei", 11, QFont.Bold))
        t_date.setStyleSheet(f"color: {c.text_primary}; background: transparent;")
        col_date.addWidget(t_date)

        def _make_date_row(label_text, default_date=None):
            row = QHBoxLayout()
            row.setSpacing(8)
            lbl = QLabel(label_text)
            lbl.setFont(QFont("Microsoft YaHei", 11))
            ...
            if default_date:
                ...
                date_edit.setDate(date_edit.date().addYears(-1))  # 默认往前1年
        ...
        r, self._bt_start_date = _make_date_row("起始", QDate.currentDate().addYears(-1))
        col_date.addLayout(r)
        r, self._bt_end_date = _make_date_row("截止", QDate.currentDate())
        col_date.addLayout(r)

        # 不限制 checkbox
        self._bt_date_unlimited = QCheckBox("不限制")
        self._bt_date_unlimited.setChecked(True)
        self._bt_date_unlimited.setFont(QFont("Microsoft YaHei", 10))
        self._bt_date_unlimited.setStyleSheet(f"color: {c.text_secondary}; background: transparent;")
        self._bt_date_unlimited.toggled.connect(self._on_date_unlimited_toggled)
        col_date.addWidget(self._bt_date_unlimited)

        row_bottom.addLayout(col_date)
        ...
        row_bottom.addLayout(col5)

        body.addWidget(grid)

    def _on_date_unlimited_toggled(self, checked):
        """切换日期限制开关"""
        self._bt_start_date.setEnabled(not checked)
        self._bt_end_date.setEnabled(not checked)
        ...
        try:
            ...
            for code, name, volume, cost in positions:
                ...
                try:
                    ...
                    start_date = None
                    end_date = None
                    if not self._bt_date_unlimited.isChecked():
