...
class PositionPanel(QFrame):
    ...
    def __init__(self):
        """初始化"""
        super().__init__()
        self._positions = []  # [(code, name, volume, cost, fee, timestamp), ...]
        self._prices = {}
    ...
    def _init_ui(self):
        ...
        self.cost_input.setPlaceholderText("例: 12.345")
        ...
        # 手续费
        self.fee_lbl = QLabel("手续费")
        self.fee_lbl.setStyleSheet("color: #999; font-size: 12px; background: transparent;")
        self.fee_input = QLineEdit()
        self.fee_input.setPlaceholderText("例: 5.00 (可选)")
        self.fee_input.setFixedHeight(36)
        self.fee_input.setStyleSheet("""
            ...
            QLineEdit:focus { border: 1px solid #1a73e8; }
        """)
        row.addWidget(self.fee_lbl)
        row.addWidget(self.fee_input, 1)
        ...
        try:
            if os.path.exists(self.DATA_FILE):
                with open(self.DATA_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                loaded = [
                    (d["code"], d["name"], d["volume"], d["cost"],
                     d.get("fee", 0), d.get("timestamp", ""))
                    for d in data
                ]
                with self._positions_lock:
    ...
    def _save_to_file(self):
        ...
        try:
            ...
            data = [
                {"code": c, "name": n, "volume": v, "cost": p,
                 "fee": f, "timestamp": t}
                for c, n, v, p, f, t in self._positions
            ]
            # 原子写入：临时文件 + rename
    ...
    def _fetch_prices(self):
        ...
        with self._positions_lock:
            codes = list(set(c for c, _, _, _, _, _ in self._positions))
        if not codes:
            return
    ...
    def _fetch_single_price(self, code):
        ...
        if code in data:
            ...
            if price > 0:
                QMetaObject.invokeMethod(self, "_update_price_display",
                                         Qt.QueuedConnection, Q_ARG(str, f"¥ {_fmt_price(price)}"))
                return
    ...
    def _add_position(self):
        ...
        if fee_text:
            try:
                fee = float(fee_text)
                if fee < 0:
                    self._show_error("手续费不能为负数")
                    return
            except ValueError:
                self._show_error("手续费格式不正确")
                return

        # 添加时间戳（加锁保护）
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with self._positions_lock:
            self._positions.append((code, name, int(vol_text), cost, fee, timestamp))
        ...
        log.signal_log("position", f"添加持仓: {code} {name}",
                       f"{vol_text}股 @ ¥{_fmt_price(cost)}" + (f" 手续费¥{fee:.2f}" if fee > 0 else ""))
    ...
    def _refresh_table(self):
        ...
        for code, records in grouped.items():
            ...
            # 渲染该股票的每条记录
            for original_idx, (c, n, vol, cost, fee, timestamp) in records:
                if has_price:
                    value = current_price * vol
                    profit = (current_price - cost) * vol - fee
                    rate = ((current_price - cost) * vol - fee) / (cost * vol) * 100 if cost > 0 else 0
                else:
                    value = 0.0
                    profit = 0.0 - fee
                    rate = 0.0
                ...
                self._render_data_row(row, timestamp, c, n, vol, cost,
                                      ...
                                      profit, rate, fee, original_idx)
                row += 1

            # ---- 该股票小计行 ----
            stock_profit = stock_total_value - stock_total_cost - stock_total_fee
            ...
            self._render_summary_row(row, f" 小计", stock_total_vol,
                                     ...
                                     current_price if has_price else None,
                                     stock_profit, stock_rate, stock_total_fee)
            row += 1

        # ---- 全局总汇总行 ----
        global_profit = global_total_value - global_total_cost - global_total_fee
        ...
        self._render_summary_row(row, " 总汇总", global_total_vol,
                                 ...
                                 None,  # 总汇总无统一现价
                                 global_profit, global_rate, global_total_fee)

    def _render_data_row(self, row, time_str, code, name, vol, cost, current, profit, rate, fee, del_idx):
        """渲染数据行（带删除按钮）"""
        self._set_item(row, 0, time_str)
        self._set_item(row, 1, code)
        self._set_item(row, 2, name)
        self._set_item(row, 3, str(vol))
        self._set_item(row, 4, _fmt_price(cost))

        if current is not None and current > 0:
            self._set_item(row, 5, _fmt_price(current))
            p_text = f"{profit:+,.2f}"
            r_text = f"{rate:+.2f}%"
        else:
            self._set_item(row, 5, "—")
            p_text = "—" if fee == 0 else f"{profit:+,.2f}"
            r_text = "—"
    ...
    def _render_summary_row(self, row, label, vol, avg_cost, current_price, profit, rate, fee):
    ...
    def _delete_row(self, original_idx):
        ...
        with self._positions_lock:
            if not (0 <= original_idx < len(self._positions)):
                return
            code, name, vol, cost, fee, _ = self._positions[original_idx]
