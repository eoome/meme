...
class LogPanel(QFrame):
    ...
    def _init_ui(self):
        ...
        # ---- 上半: 交易日志 ----
        trade_card = QFrame()
        trade_card.setStyleSheet("QFrame { background: transparent; border: none; border-radius: 10px; }")
        ...
        # ---- 下半: 操作日志 (实时) ----
        op_card = QFrame()
        op_card.setStyleSheet("QFrame { background: transparent; border: none; border-radius: 10px; }")
    ...
    def _append_op_entry(self, entry):
        ...
        icon, text_color = self._LEVEL_STYLE.get(entry.level, ("○", "#999"))

        cat_label = Logger.get_category_label(entry.category)

        html = (
            f'<div style="margin-bottom:4px;padding:3px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'
            f'<span style="color:#bbb;font-size:13px;font-family:Consolas,monospace;">{entry.time}</span>'
            f'<span style="color:{text_color};font-size:13px;margin-left:8px;">{icon} {cat_label}</span>'
            f'<span style="margin-left:8px;color:#333;font-size:13px;">{entry.message}</span>'
        )
        if entry.detail:
            html += f'<span style="color:#888;font-size:12px;margin-left:6px;">{entry.detail}</span>'
        html += '</div>'
    ...
    def _append_trade_entry(self, entry: dict):
        ...
        html = (
            ...
            f'<span style="color:{color};font-size:13px;margin-left:8px;font-weight:bold;">'
        ...
        if reason:
            html += f'<span style="color:#888;font-size:12px;margin-left:6px;">{reason}</span>'
        html += '</div>'
