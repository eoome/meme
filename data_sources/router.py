...
def _code_to_symbol(code: str) -> str:
    ...
    8xxxxx        → bj  (北交所: 83/87 开头)
    9xxxxx        → sh  (沪市: 920xxx 等科创板/沪市新股)
...
class DataRouter:
    ...
    def get_minute(self, code: str) -> List[dict]:
        """
        获取当日实时分时数据 — 并发请求，哪个先返回用哪个

        并发策略: 同时请求东财实时 + 腾讯分时 + 东财历史，取第一个有效结果。
        比串行降级快 3-5 倍（从 ~15s 降到 ~3s）。
        ...
        # 定义并发任务（按数据质量排序，质量高的优先检查结果）
        tasks = [
            ("eastmoney_rt", lambda: fetch_minute_eastmoney_realtime(code)),
            ("tencent",      lambda: fetch_minute_tencent(code)),
            ("eastmoney_1m", lambda: fetch_minute_eastmoney(code, period=1, count=240)),
        ]
        if self._akshare_available:
            tasks.append(("akshare", lambda: fetch_minute_akshare(code, period="5")))
            ...
            for future in as_completed(futures, timeout=self.GLOBAL_TIMEOUT):
                ...
                try:
                    ...
                    if data and len(data) > len(best_data):
                        ...
                        best_source = name
                        # 东财实时数据质量最高，拿到就直接用
                        if name == "eastmoney_rt" and len(data) >= 10:
                            break
                except Exception as e:
        ...
        if best_data:
            if elapsed > 3:
                log.debug("data", f"分时数据获取: {code} ({best_source}, {elapsed}s, {len(best_data)}条)")
            return _clean_minute_data(best_data)

        # 全部失败
        log.warning("data", f"分时数据全部失败: {code} ({elapsed}s)",
                    "; ".join(errors[:3]) if errors else "无数据源返回")
        return []
