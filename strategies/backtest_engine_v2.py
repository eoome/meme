...
class BacktestConfig:
    ...
    def __init__(self, **overrides):
        ...
        # 回测起止日期 (None 表示不限制)
        self.start_date = overrides.get('start_date', None)  # 格式: "2024-01-01"
        self.end_date = overrides.get('end_date', None)      # 格式: "2024-12-31"
...
class EnhancedBacktestResult:
    ...
    # 信号来源统计
    signal_source_stats: Dict[str, int] = field(default_factory=dict)  # {'ml': N, 'rule': N, 'default': N}
    ml_signal_ratio: float = 0.0  # ML 信号占比
...
class EnhancedBacktestEngine:
    ...
    def _run_backtest(self, df: pd.DataFrame, model, position_sizer, stop_loss,
        ...
        if self.config.start_date or self.config.end_date:
            ...
            if self.config.start_date:
                df = df[ts_str >= self.config.start_date]
            if self.config.end_date:
                df = df[ts_str <= self.config.end_date + " 23:59"]
            df = df.reset_index(drop=True)
            if len(df) < feature_window:
                logger.warning(f"日期筛选后数据不足: {len(df)} 条 < {feature_window} 条窗口")
                return result
        ...
        # ── 信号来源统计 ──
        source_counts = {'ml': 0, 'rule': 0, 'default': 0, 'unknown': 0}
        total_predictions = 0
        ...
        for idx in range(len(df)):
            ...
            # 记录信号来源 (ml / rule / default)
            pred_source = proba.get('_source', 'unknown')
            ...
            # 统计每次预测的来源
            total_predictions += 1
            source_counts[pred_source] = source_counts.get(pred_source, 0) + 1
            ...
            if stop_loss_active and position > 0:
                ...
                if sl_result.should_exit:
                    ...
                    pnl = self._execute_sell(position, exec_price, avg_cost,
                                             trades, ts, "止损", signal_source="stop_loss")
            ...
            if take_profit_active and position > 0:
                ...
                if tp_result.should_exit or tp_result.should_reduce:
                    ...
                    pnl = self._execute_sell(sell_qty, exec_price, avg_cost,
                                             trades, ts, "止盈", signal_source="take_profit")
            ...
            if signal == 'BUY':
                ...
                exec_price = price * (1 - self.config.slippage)
                pnl = self._execute_sell(sell_qty, exec_price, avg_cost,
                                         trades, ts, "信号", signal_source=pred_source)
                capital += sell_qty * exec_price
                daily_pnl.append(pnl)
                portfolio_risk.record_trade(
                    pnl=pnl, is_close=True, code=code, direction='sell',
        ...
        # ── 信号来源统计 ──
        result.signal_source_stats = dict(source_counts)
        if total_predictions > 0:
            result.ml_signal_ratio = source_counts.get('ml', 0) / total_predictions
        # ── 记录实际回测日期范围 ──
        if len(df) > 0:
            result.backtest_start_date = str(df['timestamp'].iloc[0])[:10]
            result.backtest_end_date = str(df['timestamp'].iloc[-1])[:10]
        return result
    ...
    def _execute_buy(self, shares: int, exec_price: float, trades: List, ts: str,
                     signal_source: str = 'unknown') -> float:
        """
