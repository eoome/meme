...
class MLModel:
    ...
    def predict(self, sample) -> Dict[str, float]:
        ...
        if expected_dim > 0 and len(sample) != expected_dim:
            ...
            result = self._rule_predict(sample)
            result['_source'] = 'rule'
            return result
        ...
        if self._lgb_model is not None:
            try:
                ...
                result = self._proba_to_dict(proba)
                result['_source'] = 'ml'
                return result
    ...
    def _rule_predict(self, sample: np.ndarray) -> Dict[str, float]:
        ...
        使用 FEATURE_COLS 名称动态查找索引，避免硬编码位置导致的
        特征顺序变化后静默错误。
        ...
        try:
            from strategies.data.features import FEATURE_COLS, FEATURE_COLS_EXTENDED
            n = len(sample)
            n_feat = len(FEATURE_COLS)

            # 动态查找特征在展平向量中的位置
            def _get_feat(name: str, feat_cols: list, sample_len: int) -> Optional[float]:
                """从展平向量中提取最新时间步的特征值"""
                if name not in feat_cols:
                    return None
                idx = feat_cols.index(name)
                # 展平格式: [feat0_t0, feat0_t1, ..., feat0_tN, feat1_t0, ...]
                # 最新时间步 = 每个特征的最后一段
                lookback = sample_len // len(feat_cols) if len(feat_cols) > 0 else 20
                pos = idx * lookback + (lookback - 1)
                if pos < sample_len:
                    return float(sample[pos])
                return None

            # 优先用扩展特征集，回退到基础
            feat_cols = FEATURE_COLS_EXTENDED if n >= len(FEATURE_COLS_EXTENDED) * (n // len(FEATURE_COLS_EXTENDED)) else FEATURE_COLS

            ret = _get_feat('feat_return', feat_cols, n)
            vwap_dev = _get_feat('feat_vwap_dev', feat_cols, n)
            ma_aligned = _get_feat('feat_ma_aligned', feat_cols, n)
            vol_ratio = _get_feat('feat_volume_ratio', feat_cols, n)
            rsi = _get_feat('feat_rsi14', feat_cols, n)
            macd_hist = _get_feat('feat_macd_hist', feat_cols, n)
            bb_pos = _get_feat('feat_bb_position', feat_cols, n)

            # ── 买入评分 ──
            if ret is not None and vwap_dev is not None:
                if ret > 0 and vwap_dev > 0:
                    buy_score += 0.30
            if ma_aligned is not None and ma_aligned > 0.5:
                buy_score += 0.20
            if vol_ratio is not None and vol_ratio > 1.5:
                buy_score += 0.12
            if rsi is not None and rsi < 0.30:
                buy_score += 0.15
            if macd_hist is not None and macd_hist > 0:
                buy_score += 0.10
            if bb_pos is not None and bb_pos < 0.2:
                buy_score += 0.08  # 接近布林带下轨，超卖
    ...
    def _safe_default() -> Dict[str, float]:
        """安全的默认概率（无法预测时返回）"""
        return {'BUY': 0.3, 'SELL': 0.3, 'NONE': 0.4, '_source': 'default'}
