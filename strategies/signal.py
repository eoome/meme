...
class Signal:
    ...
    def get_explanation(self) -> Dict:
        ...
        if proba_buy > 0 or proba_sell > 0:
            ml_dir = '买入' if proba_buy > proba_sell else ('卖出' if proba_sell > proba_buy else '中性')
        ...
        if cl_signal != 'HOLD' or cl_reason:
            cl_dir = '买入' if cl_signal == 'BUY' else ('卖出' if cl_signal == 'SELL' else '中性')
            # 买卖点级别
        ...
        if regime:
            ...
            regime_label, regime_dir = regime_map.get(regime, (regime, '中性'))
            factors.append({
                'icon': '🌡️',
                'label': '市场状态',
